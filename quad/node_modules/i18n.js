...
let availableTranslations = fs.readdirSync(process.env["QUAD_TR_DIR"]);
i18next.use(i18nextbackend).init({
    ...
    interpolation: {
        format: function fmt(value, format, lng) {
            ...
            if (format === "dp1") return value.toFixed(1);
            return value;
        },
        escapeValue: false
    }
...
async function trForDescriptor(translator) {
    ...
    if (translator.constructor === Eris.Guild || translator.constructor === Eris.User) {
        //Get the translation for a Discord ID
        translator = translator.id;
    }
    
    if (translator.length === 18) {
        const locale = await db.getLocale(translator)
        translator = locale || 'en'

        /* OLD CODE
        let response = await db.getPool().query("SELECT * FROM locales WHERE id=$1", [translator]);
